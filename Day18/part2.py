...
max_magnitude = -1
for addendum_1 in addenda:
    for addendum_2 in addenda:
        if addendum_1 == addendum_2: continue
        number = [copy.deepcopy(addendum_1), copy.deepcopy(addendum_2.copy())]
        #print(f"{addendum_1} + {addendum_2}")
        snailfish_apply_add(number)
        magnitude = get_magnitude(number)
        #print(f"= {number}. Magnitude {magnitude}" )
